...
int mac::get_dl_sched(uint32_t tti, dl_sched_t *dl_sched_res)
{
      ...
      for (uint32_t tb = 0; tb < SRSLTE_MAX_TB; tb++) {
        ...
        if (sched_result.data[i].nof_pdu_elems[tb] > 0) {
          ...
          if (pcap) {
            ...
            log_h->info_hex(dl_sched_res->sched_grants[n].data[tb], sched_result.data[i].tbs[tb], "PDU: pcap_write_dl_crnti rnti=0x%hx, bytes %u, tti %u\n",
                            rnti, sched_result.data[i].tbs[tb], tti);
          }
  ...
  for (uint32_t i=0;i<sched_result.nof_rar_elems;i++) {
    ...
    if (pcap) {
      ...
      log_h->info_hex(dl_sched_res->sched_grants[n].data[0], sched_result.rar[i].tbs, "PDU: pcap_write_dl_ranti rnti 0x%hx, bytes %u, tti %u\n",
           dl_sched_res->sched_grants[n].rnti, sched_result.rar[i].tbs, tti);
    }
  ...
  for (uint32_t i=0;i<sched_result.nof_bc_elems;i++) {
    ...
    if (sched_result.bc[i].type == sched_interface::dl_sched_bc_t::BCCH) {
      ...
      if (pcap) {
        ...
        log_h->info_hex(dl_sched_res->sched_grants[n].data[0], sched_result.bc[i].tbs, "PDU: pcap_write_dl_sirnti bytes %u, tti %u\n",
             sched_result.bc[i].tbs, tti);
      }
#endif
    } else {
      ...
      if (pcap) {
        ...
        log_h->info_hex(dl_sched_res->sched_grants[n].data[0], sched_result.bc[i].tbs, "PDU: pcap_write_dl_pch bytes %u, tti %u\n",
             sched_result.bc[i].tbs, tti);
      }
    }
