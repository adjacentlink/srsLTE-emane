...
#ifdef PHY_ADAPTER_ENABLE
        res = phy_adapter::enb_ul_get_pusch(&enb_ul, &phy_grant,
...
#ifdef PHY_ADAPTER_ENABLE
      float snr_db = crc_res ? 100.0 : 0.0;
#else
      ...
      */
      ...
      for (uint32_t tb = 0; tb < SRSLTE_MAX_TB; tb++) {
        if (acks_pending[tb]) {
          ...
          Info("XXX pusch tb %u, ack_idx %u, ack %d, rnti %u, snr_db %f, valid %d\n", 
               tb, ack_idx, ack, rnti, snr_db, valid);
...
int phch_worker::decode_pucch()
{
  ...
  for(std::map<uint16_t, ue>::iterator iter=ue_db.begin(); iter!=ue_db.end(); ++iter) {
    ...
    if (rnti >= SRSLTE_CRNTI_START && rnti <= SRSLTE_CRNTI_END && ue_db[rnti].has_grant_tti != (int) tti_rx) {
      ...
      if (needs_pucch) {
#ifdef PHY_ADAPTER_ENABLE
        if (phy_adapter::enb_ul_get_pucch(&enb_ul, rnti, &uci_data)) {
#else
        ...
        for (uint32_t tb = 0; tb < SRSLTE_MAX_TB; tb++) {
          if (needs_ack[tb]) {
            ...
            Info("XXX pucch, tb %u, ack_idx %u, ack %d, rnti %u, last_corr %f, valid %d\n", 
                 tb, ack_idx, ack, rnti, srslte_pucch_get_last_corr(&enb_ul.pucch), valid);
            phy->mac->ack_info(tti_rx, rnti, tb, ack && valid);
          }
        }
...
{
#ifdef PHY_ADAPTER_ENABLE
  uint32_t refid=phy_adapter::DL_DCIREF_ID_BEGIN;
#endif
  for (uint32_t i=0;i<nof_grants;i++) {
    srslte_enb_dl_pdsch_t *grant = &grants[i];
    uint16_t rnti = grant->rnti;
    if (rnti) {
      Debug("phch_worker::encode_pdcch_dl "
            "rnti=%d "
            "location.L=%d "
            "location.ncce=%d\n", rnti, grants[i].location.L, grants[i].location.ncce);
#ifdef PHY_ADAPTER_ENABLE
      if (phy_adapter::enb_dl_put_pdcch_dl(&grants[i], &enb_dl, refid)) {
#else
...
#ifdef PHY_ADAPTER_ENABLE
  phy_adapter::enb_dl_put_pmch(grant, phy_grant);
#else
...
int phch_worker::encode_pdsch(srslte_enb_dl_pdsch_t *grants, uint32_t nof_grants) {
  ...
  for (uint32_t i = 0; i < nof_grants; i++) {
    ...
    if (rnti) {
...
#ifdef PHY_ADAPTER_ENABLE
      float rho_a_db = rho_a == 1.0f ? 0.0 : 20.0f * log10f(rho_a);
      float rho_b_db = rho_b == 1.0f ? 0.0 : 20.0f * log10f(rho_b);
      phy_adapter::enb_dl_set_power_allocation(tti_tx_dl, rnti, rho_a_db,  rho_b_db);
#endif
      ...
      Debug("phch_worker::encode_pdsch "
            "rnti=%d "
            "sf_tx=%d "
            "phy->pdsch_p_b=%d "
            "rho_a=%0.2f "
            "rho_b=%0.2f\n", rnti, sf_tx, phy->pdsch_p_b, rho_a, rho_b);
