...
bool phch_worker::set_cell(srslte_cell_t cell_)
{
  ...
  if (cell.id != cell_.id || !cell_initiated) {
    ...
    if(cell.nof_prb != cell_.nof_prb) {
      log_h->console("Cell %d nof_prb %d -> %d\n", cell.id, cell.nof_prb, cell_.nof_prb);
      cell.nof_prb = cell_.nof_prb;
    }
  }
  ret = true;
unlock:
...
#ifdef PHY_ADAPTER_ENABLE
    if (phy_adapter::ue_dl_find_dl_dci(&ue_dl, dl_rnti, &dci_msg) != 1) {
#else
...
#ifdef PHY_ADAPTER_ENABLE
      bool ack = phy_adapter::ue_dl_decode_pmch(&ue_dl, mbsfn_area_id, payload);
#else
...
bool phch_worker::decode_phich(bool *ack)
{
  ...
  if (phy->get_pending_ack(tti, &I_lowest, &n_dmrs)) {
    if (ack) {
#ifdef PHY_ADAPTER_ENABLE
      *ack = phy_adapter::ue_dl_decode_phich(&ue_dl, tti%10, phy->get_dl_rnti(tti), I_lowest, n_dmrs);
#else
...
bool phch_worker::decode_pdcch_ul(mac_interface_phy::mac_grant_t* grant)
{
  ...
  if (phy->get_pending_rar(tti, &rar_grant)) {
    ...
    if (ul_rnti) {
#ifdef PHY_ADAPTER_ENABLE
      if (phy_adapter::ue_dl_find_ul_dci(&ue_dl, ul_rnti, &dci_msg) != 1) {
#else
...
#ifdef PHY_ADAPTER_ENABLE
  if (phy_adapter::ue_ul_put_pusch(rnti, grant, &uci_data, payload))
#else
...
#ifdef PHY_ADAPTER_ENABLE
    if (phy_adapter::ue_ul_put_pucch(&ue_ul, &uci_data, last_dl_pdcch_ncce)) {
#else
...
void phch_worker::update_measurements() 
{
    ...
    dl_metrics.n        = phy->avg_noise      =  0.001;
    dl_metrics.rsrp     = phy->avg_rsrp_dbm   = -50.0;
    dl_metrics.rsrq     = phy->avg_rsrq_db    =  -5.0;
    dl_metrics.rssi     = phy->avg_rssi_dbm   =  10.0;
    dl_metrics.pathloss = phy->pathloss       =  50.0;
        
    phy->avg_rsrp = 10.0;
    dl_metrics.sinr = phy->avg_snr_db_cqi = 10*log10(phy->avg_rsrp/phy->avg_noise);
  ...
  Info("measurements: chest %s, noise %6.6lf, rsrp %6.6lf, rsrp_dbm %6.6lf, rsrq_db %6.6lf, rssi_dbm %6.6lf, pataloss %6.6lf, ri %d, pmi %d, snr_db_cqi %6.6lf\n",
        chest_done ? "yes" : "no",
        phy->avg_noise,
        phy->avg_rsrp,
        phy->avg_rsrp_dbm,
