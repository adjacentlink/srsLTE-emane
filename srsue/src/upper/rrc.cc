...
void rrc::run_tti(uint32_t tti) {
  ...
  if (!pthread_mutex_trylock(&mutex)) {
    ...
    // Run state machine
    // too noisy rrc_log->debug("State %s\n", rrc_state_text[state]);
    switch (state) {
      case RRC_STATE_IDLE:
...
void rrc::set_serving_cell(uint32_t cell_idx) {
  rrc_log->warning("XXX %s cell_idx %u, nof_neighbours=%zd\n",
                   __func__, cell_idx, neighbour_cells.size());
...
void rrc::send_con_request(LIBLTE_RRC_CON_REQ_EST_CAUSE_ENUM cause) {
  ...
  if (ueIdentity_configured) {
    ...
    rrc_log->info("Preparing RRC Connection Request using tmsi 0x%x\n", ueIdentity.m_tmsi);
  } else {
    ...
    rrc_log->info("Preparing RRC Connection Request using random_id 0x%lx\n", random_id);
